...
int phi(int a);   //计算欧拉函数值

int main()
{
    cout << "计算模p^α的原根,请输入p的值(p为奇素数):";
    int p;
    cin >> p;
    cout << endl;
    if (ifsu(p) != 1 && p % 2 == 0)
    ...
    if (biggest(u[0], p) == 1 && biggest(u[1], p) == 1)
        cout << "因此，g=" << g << "和g+p=" << g + p << "都是模m=" << p << "^2=" << pow(p, 2)
             << "的原根。" << endl
             << endl
             << "因此，当d遍历模φ(" << p << "^2)=" << phi(pow(p, 2)) << "的简化剩余系时,"
             << g << "^d遍历模" << p << "^2的所有原根." << endl
             << endl;
    /*
    ...
    cout << endl
         << "共φ(p-1)=" << d_n << "个数时,g^d遍历模" << p << "的所有元根." << endl
         << endl;
    for (int i = 0; i < d_n; i++)
